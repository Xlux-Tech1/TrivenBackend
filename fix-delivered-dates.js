...
export const fixDates = async () => {
  try {
    console.log('Running fixDates in background...');
    ...
    const orders = await db.collection('shipmaxxorders').find({ status: 'DELIVERED' }).toArray();
    
    console.log(`Found ${orders.length} DELIVERED orders to check.`);
    ...
    for (let i=0; i<orders.length; i++) {
      ...
      try {
        ...
        console.error(`Error fixing AWB ${o.awb_code}:`, msg);
      }
    ...
    return { fixedCount: 0, errors: [e.message] };
  }
};
