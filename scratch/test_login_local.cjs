...
async function run() {
  ...
  // Test with quotes stripped if the env file has quotes
  if (password) {
    const strippedPassword = password.replace(/^'|'$/g, '').replace(/^"|"$/g, '');
  ...
  // Also try app.shipmaxx.in vs api.shipmaxx.in
  const altUrl = fullUrl.replace('app.shipmaxx.in', 'api.shipmaxx.in');
  if (altUrl !== fullUrl) {
    console.log(`\nTesting connection to alternate URL: ${altUrl}`);
    const resAlt = await post(altUrl, { email_id, password: password.replace(/^'|'$/g, '') });
    console.log(`Response status: ${resAlt.status}`);
    console.log(`Response body: ${resAlt.body}`);
  }
}
